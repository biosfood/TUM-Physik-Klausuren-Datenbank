...
const websiteUrl = 'https://www.ph.tum.de/academics/bsc/break/';
...
async function scrapePDFLinks(url) {
    ...
    const pdfLinks = await page.evaluate(() => {
        const links = Array.from(document.getElementsByClassName("accordion")[1].querySelectorAll('a'));
        let pdfLinks = [];
...
async function scrapeAndDownloadPDFs(url, directory) {
    ...
    for (const link of pdfLinks) {
        ...
        if (fileName.endsWith("solution.pdf")) {
